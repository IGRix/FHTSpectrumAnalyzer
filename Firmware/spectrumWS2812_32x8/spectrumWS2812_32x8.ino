...
// цвета высоты полос спектра. Цвета из 3 цветов (типа 0x112233) устанавливать нежелательно (приводит к срыву работы программы)
#define COLOR1 CRGB::Green
#define COLOR2 CRGB::Yellow
#define COLOR3 CRGB::OrangeRed
...
// анимация
#define SMOOTH 0.8        // плавность движения столбиков (0 - 1)
#define DELAY 5           // задержка между обновлениями матрицы (периодичность основного цикла), миллиисекунды
...
#define AUTO_GAIN 0       // автонастройка по громкости (экспериментальная функция) (1 вкл, 0 выкл)
...
#define FALL_PAUSE 150    // пауза перед падением точек максимума, миллисекунды

// массив тонов, расположены примерно по параболе. От 80 Гц до 16 кГц
//byte posOffset[17] = {2, 3, 4, 6, 8, 10, 12, 14, 16, 20, 25, 30, 35, 60, 80, 100, 120}; // частотный ряд для <17 столбцов

//byte posOffset[25] = {1, 2, 3, 4, 5, 6, 8, 10, 12, 14, 16, 20, 24, 28, 34, 40, 48, 54, 60, 70, 80, 90, 100, 110, 120}; //частотный ряд <25 столбцов
byte posOffset[33] = {1, 2, 3, 4, 5, 6, 7, 8, 10, 12, 14, 16, 18, 20, 22, 24, 28, 32, 36, 40, 46, 50, 56, 62, 68, 74, 80, 86, 92, 100, 108, 114, 120}; //частотный ряд для <33 столбцов
// -------------------------------- НАСТРОЙКИ --------------------------------
...
void loop() {
  if (millis() - mainDelay > DELAY) {     // итерация главного цикла
    ...
    for (byte pos = 0; pos < WIDTH; pos++) {    // для кажого столбца матрицы
      ...
      // если точка максимума выше нуля (или равна ему) и отрисовка точек_максимума включена - включить пиксель
